...
public class ConfigManager implements Util {
    ...
    public String config = "meowhook/config/";
    ...
    public static void setValueFromJson(Feature feature, Setting setting, JsonElement element) {
        ...
        MeowHook.LOGGER.error("Unknown Setting type for: " + feature.getName() + " : " + setting.getName());
    }
    ...
    private static void loadFile(JsonObject input, Feature feature) {
        for (Map.Entry<String, JsonElement> entry : input.entrySet()) {
            ...
            if (feature instanceof FriendManager) {
                try {
                    MeowHook.friendManager.addFriend(new FriendManager.Friend(element.getAsString(), UUID.fromString(settingName)));
                } catch (Exception e) {
                    e.printStackTrace();
                }
                continue;
    ...
    public void loadConfig(String name) {
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("meowhook").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        if (files.contains(new File("meowhook/" + name + "/"))) {
            this.config = "meowhook/" + name + "/";
        } else {
            this.config = "meowhook/config/";
        }
    ...
    public boolean configExists(String name) {
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("meowhook").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        return files.contains(new File("meowhook/" + name + "/"));
    }
    ...
    public void saveConfig(String name) {
        ...
        if (!path.exists())
            path.mkdir();
        MeowHook.friendManager.saveFriends();
        for (Feature feature : this.features) {
            try {
    ...
    public void saveCurrentConfig() {
        File currentConfig = new File("coune/currentconfig.txt");
        try {
            if (currentConfig.exists()) {
                FileWriter writer = new FileWriter(currentConfig);
                String tempConfig = this.config.replaceAll("/", "");
                writer.write(tempConfig.replaceAll("meowhook", ""));
                writer.close();
            } else {
    ...
    public String loadCurrentConfig() {
        File currentConfig = new File("meowhook/currentconfig.txt");
        String name = "config";
        try {
            if (currentConfig.exists()) {
    ...
    public void init() {
        this.features.addAll(MeowHook.moduleManager.modules);
        this.features.add(MeowHook.friendManager);
        String name = loadCurrentConfig();
        loadConfig(name);
        MeowHook.LOGGER.info("Config loaded.");
    }
    ...
    private void loadPath(Path path, Feature feature) throws IOException {
        ...
        try {
            ...
            MeowHook.LOGGER.error("Bad Config File for: " + feature.getName() + ". Resetting...");
