...
import meow.miyarishu.meowhook.features.modules.combat.UnicodeLagger;
import meow.miyarishu.meowhook.util.Timer;
import meow.miyarishu.meowhook.event.events.*;
...
public class EventManager extends Feature {
    ...
    public void onUpdate(LivingEvent.LivingUpdateEvent event) {
        if (!fullNullCheck() && (event.getEntity().getEntityWorld()).isRemote && event.getEntityLiving().equals(mc.player)) {
            MeowHook.inventoryManager.update();
            MeowHook.moduleManager.onUpdate();
            if ((HUD.getInstance()).renderingMode.getValue() == HUD.RenderingMode.Length) {
                MeowHook.moduleManager.sortModules(true);
            } else {
                MeowHook.moduleManager.sortModulesABC();
            }
        }
    }
    ...
    public void onClientConnect(FMLNetworkEvent.ClientConnectedToServerEvent event) {
        ...
        MeowHook.moduleManager.onLogin();
    }

    @SubscribeEvent
    public void onClientDisconnect(FMLNetworkEvent.ClientDisconnectionFromServerEvent event) {
        MeowHook.moduleManager.onLogout();
    }
    ...
    public void onUpdateWalkingPlayer(UpdateWalkingPlayerEvent event) {
        if (fullNullCheck())
            return;
        if (event.getStage() == 0) {
            MeowHook.speedManager.updateValues();
            MeowHook.rotationManager.updateRotations();
            MeowHook.positionManager.updatePosition();
        }
        if (event.getStage() == 1) {
            MeowHook.rotationManager.restoreRotations();
            MeowHook.positionManager.restorePosition();
        }
    }
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        if (event.getStage() != 0)
            return;
        MeowHook.serverManager.onPacketReceived();
        if (event.getPacket() instanceof SPacketEntityStatus) {
            ...
            if (packet.getOpCode() == 35 && packet.getEntity(mc.world) instanceof EntityPlayer) {
                ...
                UnicodeLagger.getInstance().onTotemPop(player);
            }
        }
    ...
    public void onWorldRender(RenderWorldLastEvent event) {
        if (event.isCanceled())
            return;
        mc.profiler.startSection("coune");
        ...
        MeowHook.moduleManager.onRender3D(render3dEvent);
        GlStateManager.glLineWidth(1.0F);
    ...
    public void renderHUD(RenderGameOverlayEvent.Post event) {
        if (event.getType() == RenderGameOverlayEvent.ElementType.HOTBAR)
            MeowHook.textManager.updateResolution();
    }
    ...
    public void onRenderGameOverlayEvent(RenderGameOverlayEvent.Text event) {
        if (event.getType().equals(RenderGameOverlayEvent.ElementType.TEXT)) {
            ...
            MeowHook.moduleManager.onRender2D(render2DEvent);
    ...
    public void onKeyInput(InputEvent.KeyInputEvent event) {
        if (Keyboard.getEventKeyState())
            MeowHook.moduleManager.onKeyPressed(Keyboard.getEventKey());
    }
        ...
        if (event.getMessage().startsWith(Command.getCommandPrefix())) {
            ...
            try {
                ...
                if (event.getMessage().length() > 1) {
                    MeowHook.commandManager.executeCommand(event.getMessage().substring(Command.getCommandPrefix().length() - 1));
                } else {
