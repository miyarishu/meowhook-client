...
public class RenderUtil
    ...
    public static void drawBox2(BlockPos pos, Color color, double growAmt) {
        AxisAlignedBB bb = new AxisAlignedBB((double) pos.getX() - mc.getRenderManager().viewerPosX, (double) pos.getY() - mc.getRenderManager().viewerPosY, (double) pos.getZ() - mc.getRenderManager().viewerPosZ, (double) (pos.getX() + 1) - mc.getRenderManager().viewerPosX, (double) (pos.getY() + 1) - mc.getRenderManager().viewerPosY, (double) (pos.getZ() + 1) - mc.getRenderManager().viewerPosZ).grow(-growAmt);
        camera.setPosition(Objects.requireNonNull(mc.getRenderViewEntity()).posX, mc.getRenderViewEntity().posY, mc.getRenderViewEntity().posZ);
        if (camera.isBoundingBoxInFrustum(new AxisAlignedBB(bb.minX + mc.getRenderManager().viewerPosX, bb.minY + mc.getRenderManager().viewerPosY, bb.minZ + mc.getRenderManager().viewerPosZ, bb.maxX + mc.getRenderManager().viewerPosX, bb.maxY + mc.getRenderManager().viewerPosY, bb.maxZ + mc.getRenderManager().viewerPosZ))) {
            GlStateManager.pushMatrix();
            GlStateManager.enableBlend();
    ...
    public static void drawBlockOutline2(BlockPos pos, Color color, float linewidth, boolean air, double growAmt) {
        ...
        if ((air || iblockstate.getMaterial() != Material.AIR) && mc.world.getWorldBorder().contains(pos)) {
            assert (mc.renderViewEntity != null);
            Vec3d interp = EntityUtil.interpolateEntity(mc.renderViewEntity, mc.getRenderPartialTicks());
            RenderUtil.drawBlockOutline(iblockstate.getSelectedBoundingBox(mc.world, pos).grow(-growAmt).offset(-interp.x, -interp.y, -interp.z), color, linewidth);
        }
    }
    ...
    public static void drawText(BlockPos pos, String text) {
        ...
        GlStateManager.disableDepth();
        GlStateManager.translate(-((double) MeowHook.textManager.getStringWidth(text) / 2.0), 0.0, 0.0);
        MeowHook.textManager.drawStringWithShadow(text, 0.0f, 0.0f, -5592406);
        GlStateManager.popMatrix();
    }
