...
public class MovementUtil implements Util {
    ...
    public static MovementInput inverse(Entity entity, double speed) {
        ...
        for (float d = -1.0f; d <= 1.0f; d += 1.0f) {
            for (float e = -1.0f; e <= 1.0f; e += 1.0f) {
                ...
                if ((targetMotionX < 0 ? entity.motionX <= targetMotionX : entity.motionX >= targetMotionX) && (targetMotionZ < 0 ? entity.motionZ <= targetMotionZ : entity.motionZ >= targetMotionZ)) {
                    input.moveForward = d;
                    input.moveStrafe = e;
                    break;
                }
    ...
    public static void step(float height) {
        if (mc.player.isOnLadder()) { //removed !collidedVerti and !onGround and (double) mc.player.fallDistance > 0.1
            return;
        }
    ...
    public static void setMotion(double x, double y, double z) {
        if (mc.player != null) {
            if (mc.player.isRiding()) {
                ...
                mc.player.ridingEntity.motionZ = x;
            } else {
                mc.player.motionX = x;
                mc.player.motionY = y;
                mc.player.motionZ = z;
