...
import meow.miyarishu.meowhook.features.modules.combat.Killaura;
...
public class EntityUtil
    ...
    public static boolean isCrystalAtFeet(final EntityEnderCrystal crystal, final double range) {
        for (final EntityPlayer player : EntityUtil.mc.world.playerEntities) {
            ...
            if (MeowHook.friendManager.isFriend(player)) {
                continue;
            }
...
//        Block block = mc.world.getBlockState(new BlockPos(mc.player.getPositionVector().add(0.0, 0.0, 0.0))).getBlock();
        //       return block == Blocks.OBSIDIAN || block == Blocks.ENDER_CHEST;
        //return !(mc.world.getCollisionBoxes(entity, entity.getEntityBoundingBox().expand(0, 0, 0)).isEmpty()) && !isInterceptedByOther(entity);
    ...
    public static boolean isntValid(Entity entity, double range) {
        return entity == null || EntityUtil.isDead(entity) || entity.equals(EntityUtil.mc.player) || entity instanceof EntityPlayer && MeowHook.friendManager.isFriend(entity.getName()) || EntityUtil.mc.player.getDistanceSq(entity) > MathUtil.square(range);
    }
    ...
    public static Color getColor(Entity entity, int red, int green, int blue, int alpha, boolean colorFriends) {
        ...
        if (entity instanceof EntityPlayer && colorFriends && MeowHook.friendManager.isFriend((EntityPlayer) entity)) {
            color = new Color(0.33333334f, 1.0f, 1.0f, (float) alpha / 255.0f);
        }
        final Killaura killaura = MeowHook.moduleManager.getModuleByClass(Killaura.class);
    ...
    public static Map<String, Integer> getTextRadarPlayers() {
        ...
        for (final EntityPlayer player : EntityUtil.mc.world.playerEntities) {
            ...
            if (hpRaw >= 20) {
                healthS = ChatFormatting.GREEN + hp + "hp" + ChatFormatting.RESET;
            } else if (hpRaw >= 10) {
                healthS = ChatFormatting.YELLOW + hp + "hp" + ChatFormatting.RESET;
            } else if (hpRaw >= 5) {
                healthS = ChatFormatting.RED + hp + "hp" + ChatFormatting.RESET;
            } else {
                healthS = ChatFormatting.DARK_RED + hp + "hp" + ChatFormatting.RESET;
            }
            final int distanceInt = (int) EntityUtil.mc.player.getDistance(player);
            final String distance = dfDistance.format(distanceInt);
            if (distanceInt >= 25) {
                distanceS = ChatFormatting.GREEN + distance + "m" + ChatFormatting.RESET;
            } else if (distanceInt > 10) {
                distanceS = ChatFormatting.YELLOW + distance + "m" + ChatFormatting.RESET;
            } else if (distanceInt >= 50) {
                distanceS = ChatFormatting.GRAY + distance + "m" + ChatFormatting.RESET;
            } else {
                distanceS = ChatFormatting.RED + distance + "m" + ChatFormatting.RESET;
            }
            output.put((MeowHook.friendManager.isFriend(player) ? ChatFormatting.AQUA : ChatFormatting.RED) + player.getName() + ChatFormatting.RESET + " ["  + healthS + "] [" + distanceS + "]", (int) EntityUtil.mc.player.getDistance(player));
        }
        if (!output.isEmpty()) {
