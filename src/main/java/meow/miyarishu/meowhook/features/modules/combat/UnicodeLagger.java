...
import meow.miyarishu.meowhook.event.events.ConnectionEvent;
...
public class UnicodeLagger
        extends Module {
    private static UnicodeLagger INSTANCE = new UnicodeLagger();
    public Setting<Boolean> ignore = register(new Setting("Ignore", true));
    public Setting<Boolean> lag = register(new Setting("Lag", true));
    public Setting<Integer> pop_delay = register(new Setting("Delay", 2500, 0, 10000, v -> this.lag.getValue()));
    ...
    public UnicodeLagger() {
        super("Unicode Lag", "Lags out peoples games with Unicode text", Category.COMBAT, true, false, false);
        this.setInstance();
    }

    public static UnicodeLagger getInstance() {
        if (INSTANCE == null) {
            INSTANCE = new UnicodeLagger();
        }
        return INSTANCE;
    }
    ...
    public void onReceive(PacketEvent.Receive e) {
        ...
        if (e.getPacket() instanceof SPacketChat && this.ignore.getValue()) {
            ...
            if (this.hasUnicode(chat)) {
                e.setCanceled(true);
                Command.sendMessage(ChatFormatting.RED + "Unicode detected!");
            }
        }
    }
    ...
    public void onTotemPop(EntityPlayer player) {
        if (player.entityId != this.mc.player.entityId && !MeowHook.friendManager.isFriend(player) && !this.sent.contains(player.getName()) && this.timer.passedMs(this.pop_delay.getValue()) && this.lag.getValue() && this.isOn()) {
