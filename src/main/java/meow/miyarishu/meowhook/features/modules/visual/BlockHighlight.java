...
public class BlockHighlight
    ...
    public void onRender3D(Render3DEvent event) {
        ...
        if (ray != null && ray.typeOfHit == RayTraceResult.Type.BLOCK) {
            ...
            RenderUtil.drawBlockOutline(blockpos, Colors.getInstance().rainbow.getValue() != false ? ColorUtil.rainbow(Colors.getInstance().rainbowHue.getValue()) : new Color(Colors.getInstance().red.getValue(), Colors.getInstance().green.getValue(), Colors.getInstance().blue.getValue(), this.cAlpha.getValue()), this.lineWidth.getValue().floatValue(), false);
        }
    }
}
