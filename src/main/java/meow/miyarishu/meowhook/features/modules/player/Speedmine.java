...
import meow.miyarishu.meowhook.event.events.BlockEvent;
...
public class Speedmine
    ...
    public Setting<Boolean> packetSwap = this.register(new Setting<Boolean>("Packet Swap", true));
    ...
    public double growAmt = 0.35;
    private int oldSlot = -1;
    private int swapBackDelay = 0;
    private boolean doSwap = false, swapBack = false;
    ...
    public void onTick() {
        ...
        try {
            ...
            if (this.currentPos != null) {
                ...
                if (this.timer.passedMs((int) (2000.0f * MeowHook.serverManager.getTpsFactor())) && packetSwap.getValue() && !swapBack)
                    doSwap = true;
            }
            ...
            if (packetSwap.getValue()) {
                ...
                if (doSwap) {
                    ...
                    int newSlot = InventoryUtil.getItemHotbar(Items.DIAMOND_PICKAXE);
                    if (newSlot != -1) {
                        InventoryUtil.mc.player.connection.sendPacket(new CPacketHeldItemChange(newSlot));
                        swapBackDelay = 0;
                        swapBack = true;
                        doSwap = false;
                        return;
    ...
    public void onRender3D(Render3DEvent event) {
        if (this.render.getValue().booleanValue() && this.currentPos != null && this.currentBlockState.getBlock() == Blocks.OBSIDIAN) {
            ...
            if(growAmt > 0){
                growAmt -= timer.getPassedTimeMs()*0.000004;
            }

            Color color = new Color(this.timer.passedMs((int) (2000.0f * MeowHook.serverManager.getTpsFactor())) ? 0 : 255, this.timer.passedMs((int) (2000.0f * MeowHook.serverManager.getTpsFactor())) ? 255 : 0, 0, 255);
            RenderUtil.drawBoxESP2(this.currentPos, color, false, color, this.lineWidth.getValue().floatValue(), this.outline.getValue(), this.box.getValue(), this.boxAlpha.getValue(), false, growAmt);
        }
    }
