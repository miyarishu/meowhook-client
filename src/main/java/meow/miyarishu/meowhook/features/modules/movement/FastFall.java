...
public class FastFall extends Module {
    ...
    private final Setting<Integer> height = this.register(new Setting<Integer>("Height", 10, 1, 20));
    private final Setting<Mode> mode = this.register(new Setting("Mode", Mode.FAST));
    private final Setting<Boolean> noLag = this.register(new Setting<Boolean>("NoLag", true, v -> mode.getValue() == Mode.FAST));
    ...
    public FastFall() {
        super("FastFall", "fall faster.", Category.MOVEMENT, true, false, false);
    }
        ...
        if (mode.getValue() == Mode.STRICT) {
            ...
            if (!mc.player.onGround) {
                ...
                if (mc.player.motionY < 0 && useTimer) {
                    MeowHook.timerManager.setTimer(2.5f);
                    return;
