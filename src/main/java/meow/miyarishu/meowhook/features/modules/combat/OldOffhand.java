...
import meow.miyarishu.meowhook.event.events.ProcessRightClickBlockEvent;
import meow.miyarishu.meowhook.features.modules.Module;
import meow.miyarishu.meowhook.features.setting.Bind;
import meow.miyarishu.meowhook.features.setting.EnumConverter;
...
public class OldOffhand
        extends Module {
    private static OldOffhand instance;
    ...
    public OldOffhand() {
        super("OldOffhand", "This offhand is bad and will be removed soon", Module.Category.COMBAT, true, false, false);
        instance = this;
    }

    public static OldOffhand getInstance() {
        if (instance == null) {
            instance = new OldOffhand();
        }
        return instance;
    }

    public void onItemFinish(ItemStack stack, EntityLivingBase base) {
        if (this.noOffhandGC.getValue().booleanValue() && base.equals(OldOffhand.mc.player) && stack.getItem() == OldOffhand.mc.player.getHeldItemOffhand().getItem()) {
    ...
    @Override
    public void onTick() {
        if (OldOffhand.nullCheck() || this.updates.getValue() == 1) {
            return;
        }
        this.doOffhand();
    }
    ...
    public void onUpdateWalkingPlayer(ProcessRightClickBlockEvent event) {
        if (this.noOffhandGC.getValue().booleanValue() && event.hand == EnumHand.MAIN_HAND && event.stack.getItem() == Items.END_CRYSTAL && OldOffhand.mc.player.getHeldItemOffhand().getItem() == Items.GOLDEN_APPLE && OldOffhand.mc.objectMouseOver != null && event.pos == OldOffhand.mc.objectMouseOver.getBlockPos()) {
            event.setCanceled(true);
            OldOffhand.mc.player.setActiveHand(EnumHand.OFF_HAND);
            OldOffhand.mc.playerController.processRightClick(OldOffhand.mc.player, OldOffhand.mc.world, EnumHand.OFF_HAND);
        }
    }
    ...
    public void onUpdate() {
        if (this.noOffhandGC.getValue().booleanValue() && this.retardOGC.getValue().booleanValue()) {
            if (this.timer.passedMs(this.timeout.getValue().intValue())) {
                if (OldOffhand.mc.player != null && OldOffhand.mc.player.getHeldItemOffhand().getItem() == Items.GOLDEN_APPLE && OldOffhand.mc.player.getHeldItemMainhand().getItem() == Items.END_CRYSTAL && Mouse.isButtonDown(1)) {
                    OldOffhand.mc.player.setActiveHand(EnumHand.OFF_HAND);
                    OldOffhand.mc.gameSettings.keyBindUseItem.pressed = Mouse.isButtonDown(1);
                }
            } else if (OldOffhand.mc.player.getHeldItemOffhand().getItem() == Items.GOLDEN_APPLE && OldOffhand.mc.player.getHeldItemMainhand().getItem() == Items.END_CRYSTAL) {
                OldOffhand.mc.gameSettings.keyBindUseItem.pressed = false;
            }
        }
        if (OldOffhand.nullCheck() || this.updates.getValue() == 2) {
            return;
        }
        this.doOffhand();
    ...
    public void onPacketSend(PacketEvent.Send event) {
        if (this.noOffhandGC.getValue().booleanValue() && !OldOffhand.fullNullCheck() && OldOffhand.mc.player.getHeldItemOffhand().getItem() == Items.GOLDEN_APPLE && OldOffhand.mc.player.getHeldItemMainhand().getItem() == Items.END_CRYSTAL && OldOffhand.mc.gameSettings.keyBindUseItem.isKeyDown()) {
            ...
            if (event.getPacket() instanceof CPacketPlayerTryUseItemOnBlock) {
                ...
                if (packet2.getHand() == EnumHand.MAIN_HAND) {
                    if (this.timer.passedMs(this.timeout.getValue().intValue())) {
                        OldOffhand.mc.player.setActiveHand(EnumHand.OFF_HAND);
                        OldOffhand.mc.player.connection.sendPacket(new CPacketPlayerTryUseItem(EnumHand.OFF_HAND));
                    }
                    event.setCanceled(true);
                }
    ...
    public void doOffhand() {
        ...
        if (this.type.getValue() == Type.NEW) {
            if (OldOffhand.mc.currentScreen instanceof GuiContainer && !this.guis.getValue().booleanValue() && !(OldOffhand.mc.currentScreen instanceof GuiInventory)) {
                return;
            }
            if (this.gapSwap.getValue().booleanValue()) {
                if ((this.getSlot(Mode.GAPPLES) != -1 || OldOffhand.mc.player.getHeldItemOffhand().getItem() == Items.GOLDEN_APPLE) && OldOffhand.mc.player.getHeldItemMainhand().getItem() != Items.GOLDEN_APPLE && OldOffhand.mc.gameSettings.keyBindUseItem.isKeyDown()) {
                    this.setMode(Mode.GAPPLES);
            ...
            if (!this.shouldTotem()) {
                if (OldOffhand.mc.player.getHeldItemOffhand() == ItemStack.EMPTY || !this.isItemInOffhand()) {
                    int slot;
                    int n = slot = this.getSlot(this.mode) < 9 ? this.getSlot(this.mode) + 36 : this.getSlot(this.mode);
                    if (this.getSlot(this.mode) != -1) {
                        if (this.oldSlot != -1) {
                            OldOffhand.mc.playerController.windowClick(0, 45, 0, ClickType.PICKUP, OldOffhand.mc.player);
                            OldOffhand.mc.playerController.windowClick(0, this.oldSlot, 0, ClickType.PICKUP, OldOffhand.mc.player);
                        }
                        this.oldSlot = slot;
                        OldOffhand.mc.playerController.windowClick(0, slot, 0, ClickType.PICKUP, OldOffhand.mc.player);
                        OldOffhand.mc.playerController.windowClick(0, 45, 0, ClickType.PICKUP, OldOffhand.mc.player);
                        OldOffhand.mc.playerController.windowClick(0, slot, 0, ClickType.PICKUP, OldOffhand.mc.player);
                    }
                }
            } else if (!(this.eatingApple || OldOffhand.mc.player.getHeldItemOffhand() != ItemStack.EMPTY && OldOffhand.mc.player.getHeldItemOffhand().getItem() == Items.TOTEM_OF_UNDYING)) {
                int slot;
            ...
            this.holdingCrystal = OldOffhand.mc.player.getHeldItemOffhand().getItem() == Items.END_CRYSTAL;
            this.holdingTotem = OldOffhand.mc.player.getHeldItemOffhand().getItem() == Items.TOTEM_OF_UNDYING;
            this.holdingGapple = OldOffhand.mc.player.getHeldItemOffhand().getItem() == Items.GOLDEN_APPLE;
            this.holdingObby = InventoryUtil.isBlock(OldOffhand.mc.player.getHeldItemOffhand().getItem(), BlockObsidian.class);
            this.holdingWeb = InventoryUtil.isBlock(OldOffhand.mc.player.getHeldItemOffhand().getItem(), BlockWeb.class);
            this.totems = OldOffhand.mc.player.inventory.mainInventory.stream().filter(itemStack -> itemStack.getItem() == Items.TOTEM_OF_UNDYING).mapToInt(ItemStack::getCount).sum();
            if (this.holdingTotem) {
                this.totems += OldOffhand.mc.player.inventory.offHandInventory.stream().filter(itemStack -> itemStack.getItem() == Items.TOTEM_OF_UNDYING).mapToInt(ItemStack::getCount).sum();
            }
            this.crystals = OldOffhand.mc.player.inventory.mainInventory.stream().filter(itemStack -> itemStack.getItem() == Items.END_CRYSTAL).mapToInt(ItemStack::getCount).sum();
            if (this.holdingCrystal) {
                this.crystals += OldOffhand.mc.player.inventory.offHandInventory.stream().filter(itemStack -> itemStack.getItem() == Items.END_CRYSTAL).mapToInt(ItemStack::getCount).sum();
            }
            this.gapples = OldOffhand.mc.player.inventory.mainInventory.stream().filter(itemStack -> itemStack.getItem() == Items.GOLDEN_APPLE).mapToInt(ItemStack::getCount).sum();
            if (this.holdingGapple) {
                this.gapples += OldOffhand.mc.player.inventory.offHandInventory.stream().filter(itemStack -> itemStack.getItem() == Items.GOLDEN_APPLE).mapToInt(ItemStack::getCount).sum();
            }
            if (this.currentMode == Mode2.WEBS || this.currentMode == Mode2.OBSIDIAN) {
                this.obby = OldOffhand.mc.player.inventory.mainInventory.stream().filter(itemStack -> InventoryUtil.isBlock(itemStack.getItem(), BlockObsidian.class)).mapToInt(ItemStack::getCount).sum();
                if (this.holdingObby) {
                    this.obby += OldOffhand.mc.player.inventory.offHandInventory.stream().filter(itemStack -> InventoryUtil.isBlock(itemStack.getItem(), BlockObsidian.class)).mapToInt(ItemStack::getCount).sum();
                }
                this.webs = OldOffhand.mc.player.inventory.mainInventory.stream().filter(itemStack -> InventoryUtil.isBlock(itemStack.getItem(), BlockWeb.class)).mapToInt(ItemStack::getCount).sum();
                if (this.holdingWeb) {
                    this.webs += OldOffhand.mc.player.inventory.offHandInventory.stream().filter(itemStack -> InventoryUtil.isBlock(itemStack.getItem(), BlockWeb.class)).mapToInt(ItemStack::getCount).sum();
                }
            }
            this.doSwitch();
        }
    ...
    public void doSwitch() {
        if (this.autoGapple.getValue().booleanValue()) {
            if (OldOffhand.mc.gameSettings.keyBindUseItem.isKeyDown()) {
                if (OldOffhand.mc.player.getHeldItemMainhand().getItem() instanceof ItemSword && (!this.onlyWTotem.getValue().booleanValue() || OldOffhand.mc.player.getHeldItemOffhand().getItem() == Items.TOTEM_OF_UNDYING)) {
                    this.setMode(Mode.GAPPLES);
        ...
        if (this.currentMode == Mode2.GAPPLES && ((!EntityUtil.isSafe(OldOffhand.mc.player) || this.bedPlaceable()) && EntityUtil.getHealth(OldOffhand.mc.player, this.absorption.getValue()) <= this.gappleHealth.getValue().floatValue() || EntityUtil.getHealth(OldOffhand.mc.player, this.absorption.getValue()) <= this.gappleHoleHealth.getValue().floatValue()) || this.currentMode == Mode2.CRYSTALS && ((!EntityUtil.isSafe(OldOffhand.mc.player) || this.bedPlaceable()) && EntityUtil.getHealth(OldOffhand.mc.player, this.absorption.getValue()) <= this.crystalHealth.getValue().floatValue() || EntityUtil.getHealth(OldOffhand.mc.player, this.absorption.getValue()) <= this.crystalHoleHealth.getValue().floatValue()) || this.currentMode == Mode2.OBSIDIAN && ((!EntityUtil.isSafe(OldOffhand.mc.player) || this.bedPlaceable()) && EntityUtil.getHealth(OldOffhand.mc.player, this.absorption.getValue()) <= this.obsidianHealth.getValue().floatValue() || EntityUtil.getHealth(OldOffhand.mc.player, this.absorption.getValue()) <= this.obsidianHoleHealth.getValue().floatValue()) || this.currentMode == Mode2.WEBS && ((!EntityUtil.isSafe(OldOffhand.mc.player) || this.bedPlaceable()) && EntityUtil.getHealth(OldOffhand.mc.player, this.absorption.getValue()) <= this.webHealth.getValue().floatValue() || EntityUtil.getHealth(OldOffhand.mc.player, this.absorption.getValue()) <= this.webHoleHealth.getValue().floatValue())) {
            if (this.returnToCrystal.getValue().booleanValue() && this.currentMode == Mode2.CRYSTALS) {
                this.switchedForHealthReason = true;
            }
            this.setMode(Mode2.TOTEMS);
        }
        if (this.switchedForHealthReason && (EntityUtil.isSafe(OldOffhand.mc.player) && !this.bedPlaceable() && EntityUtil.getHealth(OldOffhand.mc.player, this.absorption.getValue()) > this.crystalHoleHealth.getValue().floatValue() || EntityUtil.getHealth(OldOffhand.mc.player, this.absorption.getValue()) > this.crystalHealth.getValue().floatValue())) {
            this.setMode(Mode2.CRYSTALS);
        ...
        Item currentOffhandItem = OldOffhand.mc.player.getHeldItemOffhand().getItem();
        switch (this.currentMode) {
            case TOTEMS: {
    ...
    private boolean noNearbyPlayers() {
        return this.mode == Mode.CRYSTALS && OldOffhand.mc.world.playerEntities.stream().noneMatch(e -> e != OldOffhand.mc.player && !MeowHook.friendManager.isFriend(e) && OldOffhand.mc.player.getDistance(e) <= this.cTargetDistance.getValue().floatValue());
    }

    private boolean isItemInOffhand() {
        switch (this.mode) {
            case GAPPLES: {
                return OldOffhand.mc.player.getHeldItemOffhand().getItem() == Items.GOLDEN_APPLE;
            }
            case CRYSTALS: {
                return OldOffhand.mc.player.getHeldItemOffhand().getItem() == Items.END_CRYSTAL;
            }
            case OBSIDIAN: {
                return OldOffhand.mc.player.getHeldItemOffhand().getItem() instanceof ItemBlock && ((ItemBlock) OldOffhand.mc.player.getHeldItemOffhand().getItem()).block == Blocks.OBSIDIAN;
            }
            case WEBS: {
                return OldOffhand.mc.player.getHeldItemOffhand().getItem() instanceof ItemBlock && ((ItemBlock) OldOffhand.mc.player.getHeldItemOffhand().getItem()).block == Blocks.WEB;
            }
        }
        return false;
    }
    ...
    private boolean isHeldInMainHand() {
        switch (this.mode) {
            case GAPPLES: {
                return OldOffhand.mc.player.getHeldItemMainhand().getItem() == Items.GOLDEN_APPLE;
            }
            case CRYSTALS: {
                return OldOffhand.mc.player.getHeldItemMainhand().getItem() == Items.END_CRYSTAL;
            }
            case OBSIDIAN: {
                return OldOffhand.mc.player.getHeldItemMainhand().getItem() instanceof ItemBlock && ((ItemBlock) OldOffhand.mc.player.getHeldItemMainhand().getItem()).block == Blocks.OBSIDIAN;
            }
            case WEBS: {
                return OldOffhand.mc.player.getHeldItemMainhand().getItem() instanceof ItemBlock && ((ItemBlock) OldOffhand.mc.player.getHeldItemMainhand().getItem()).block == Blocks.WEB;
            }
        }
        return false;
    }
    ...
    private boolean shouldTotem() {
        ...
        if (this.holeCheck.getValue().booleanValue() && EntityUtil.isInHole(OldOffhand.mc.player) && !this.bedPlaceable()) {
            return OldOffhand.mc.player.getHealth() + OldOffhand.mc.player.getAbsorptionAmount() <= this.getHoleHealth() || OldOffhand.mc.player.getItemStackFromSlot(EntityEquipmentSlot.CHEST).getItem() == Items.ELYTRA || OldOffhand.mc.player.fallDistance >= 3.0f || this.noNearbyPlayers() || this.crystalCheck.getValue() != false && this.isCrystalsAABBEmpty();
        }
        return OldOffhand.mc.player.getHealth() + OldOffhand.mc.player.getAbsorptionAmount() <= this.getHealth() || OldOffhand.mc.player.getItemStackFromSlot(EntityEquipmentSlot.CHEST).getItem() == Items.ELYTRA || OldOffhand.mc.player.fallDistance >= 3.0f || this.noNearbyPlayers() || this.crystalCheck.getValue() != false && this.isCrystalsAABBEmpty();
    }

    private boolean isNotEmpty(BlockPos pos) {
        return OldOffhand.mc.world.getEntitiesWithinAABBExcludingEntity(null, new AxisAlignedBB(pos)).stream().anyMatch(e -> e instanceof EntityEnderCrystal);
    }
    ...
    private boolean isCrystalsAABBEmpty() {
        return this.isNotEmpty(OldOffhand.mc.player.getPosition().add(1, 0, 0)) || this.isNotEmpty(OldOffhand.mc.player.getPosition().add(-1, 0, 0)) || this.isNotEmpty(OldOffhand.mc.player.getPosition().add(0, 0, 1)) || this.isNotEmpty(OldOffhand.mc.player.getPosition().add(0, 0, -1)) || this.isNotEmpty(OldOffhand.mc.player.getPosition());
    }
    ...
    int getStackSize() {
        ...
        if (this.shouldTotem()) {
            for (int i = 45; i > 0; --i) {
                if (OldOffhand.mc.player.inventory.getStackInSlot(i).getItem() != Items.TOTEM_OF_UNDYING) continue;
                size += OldOffhand.mc.player.inventory.getStackInSlot(i).getCount();
            }
        } else if (this.mode == Mode.OBSIDIAN) {
            for (int i = 45; i > 0; --i) {
                if (!(OldOffhand.mc.player.inventory.getStackInSlot(i).getItem() instanceof ItemBlock) || ((ItemBlock) OldOffhand.mc.player.inventory.getStackInSlot(i).getItem()).block != Blocks.OBSIDIAN)
                    continue;
                size += OldOffhand.mc.player.inventory.getStackInSlot(i).getCount();
            }
        } else if (this.mode == Mode.WEBS) {
            for (int i = 45; i > 0; --i) {
                if (!(OldOffhand.mc.player.inventory.getStackInSlot(i).getItem() instanceof ItemBlock) || ((ItemBlock) OldOffhand.mc.player.inventory.getStackInSlot(i).getItem()).block != Blocks.WEB)
                    continue;
                size += OldOffhand.mc.player.inventory.getStackInSlot(i).getCount();
            }
        } else {
            for (int i = 45; i > 0; --i) {
                if (OldOffhand.mc.player.inventory.getStackInSlot(i).getItem() != (this.mode == Mode.CRYSTALS ? Items.END_CRYSTAL : Items.GOLDEN_APPLE))
                    continue;
                size += OldOffhand.mc.player.inventory.getStackInSlot(i).getCount();
            }
        }
    ...
    int getSlot(Mode m) {
        ...
        if (m == Mode.OBSIDIAN) {
            ...
            for (int i = 45; i > 0; --i) {
                if (OldOffhand.mc.player.inventory.getStackInSlot(i).getItem() != (m == Mode.CRYSTALS ? Items.END_CRYSTAL : Items.GOLDEN_APPLE))
                    continue;
                slot = i;
                break;
            }
    ...
    private boolean bedPlaceable() {
        ...
        if (OldOffhand.mc.world.getBlockState(OldOffhand.mc.player.getPosition()).getBlock() != Blocks.BED && OldOffhand.mc.world.getBlockState(OldOffhand.mc.player.getPosition()).getBlock() != Blocks.AIR) {
            return false;
        }
        for (EnumFacing facing : EnumFacing.values()) {
            if (facing == EnumFacing.UP || facing == EnumFacing.DOWN || OldOffhand.mc.world.getBlockState(OldOffhand.mc.player.getPosition().offset(facing)).getBlock() != Blocks.BED && OldOffhand.mc.world.getBlockState(OldOffhand.mc.player.getPosition().offset(facing)).getBlock() != Blocks.AIR)
                continue;
            return true;
        }
        return false;
